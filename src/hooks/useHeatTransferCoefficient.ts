...
export interface ComputeHArgs {
  ...
  useSimplifiedFormula?: boolean; // использовать упрощённые формулы из задачи (по умолчанию true)
...
export const computeH = ({
  ...
  // По умолчанию используем упрощённые формулы (как в референсе).
  // Если нужно, можно переключить на более точные формулы через числа Нуссельта.
